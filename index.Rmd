...
```{r actual_run,  echo=FALSE, message=FALSE, warning=FALSE, results='hide',fig.keep='all'}
...
# unlink(temp)
shapeobj1<- sf::st_read("data/eriecounty_parcels_2019_1119/eriecounty_parcels_1119.shp")
...
shapeobj2<-st_join(shp,point_trn,join=st_contains) %>%
  ...
  aggregate( by=list(.$Program.Facility.Name),FUN=unique,do_union=TRUE) %>% 
  aggregate(by=.$geometry,FUN=unique,do_union=TRUE)
...
# #get percp data
# GHCN.prcp <- get_ghcn_daily(template = shapeobj_trim, 
#                             label = "prcp", 
#                             elements = c('prcp'))
# 
# 
# #average location percip data (could also group to monthly averages etc)
# ghcn_dat<-GHCN.prcp$tabular %>%
#     lapply("[[", "PRCP") %>% 
#     do.call("rbind",.) %>% 
#     gather(day, precip, -STATION, -YEAR, -MONTH) %>%
#     filter(!is.na(precip)) %>% 
...
#                   ) %>% 
#   st_transform(crs(shapeobj_trim)) %>% 
#   left_join(ghcn_dat,by=c("ID"="Group.1"),)
# 
# write_sf(ghcn_pts,"ghcn_pts_file.shp")

##to avoid timely download
ghcn_pts<-sf::st_read("ghcn_pts_file.shp")
...
######DEMO all this here#####
##also adds in precip data based on closest station


###error here try st_union on above set next
shapeobj_trim<-st_join(shapeobj2,ghcn_pts,join=st_nn,k=1) %>% 
  dplyr::select(Program.Facility.Name,precip,ASSESSACRE,Contaminants,Program.Type,DEC.Region,Control.Code,Site.Class) %>% 
  filter(ASSESSACRE>0) %>% 
  mutate(Score=0)
  

rm(shapeobj2, ghcn_dat,ghcn_pts,GHCN.prcp)
gc(full=TRUE, verbose=FALSE)
